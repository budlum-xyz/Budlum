...
mod tests {
    ...
    /// ARENA2 task-3 (2026-07-20): VerifyInference (0x1F) prove→verify round trip.
    /// Regression test for the former `InvalidProof` (missing selector + gas +
    /// register-bus accounting, plus spurious expansion rows). A large memory
    /// (8192, matching budlum-core's ZkVmExecutor) keeps the old expansion paths
    /// reachable, so this exercises the exact failing scenario.
    #[test]
    fn proves_verify_inference_opcode_roundtrip() {
        ...
        // Single-row opcode: 3 Loads + 1 VerifyInference + 1 Halt = 5 rows.
        assert_eq!(
            vm.trace.len(),
            5,
