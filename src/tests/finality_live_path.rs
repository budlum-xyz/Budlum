...
fn live_path_snapshot_hash_distinguishes_sets() {
    ...
    assert_ne!(hash_a, hash_b, "3 vs 4 validator setleri aynı hash olmamalı");
    assert_ne!(
        hash_a, hash_c,
        "1000 vs 2000 stake setleri aynı hash olmamalı"
    );
    assert_ne!(hash_b, hash_c, "farklı stake + farklı boyut aynı hash olmamalı");

    // AYNI set deterministik olarak aynı hash üretir.
    let hash_a2 = ValidatorSetSnapshot::compute_hash(&snap_a.validators);
    assert_eq!(
        hash_a, hash_a2,
        "compute_hash deterministik olmalı"
    );
}
